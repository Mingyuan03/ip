    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (header.equals("deadline")) {
                try {
                    Deadline deadline = new Deadline(input.substring(header.length()));
                ...
                try {
                    Event event = new Event(input.substring(header.length()));
                    TASKLOGS.addTask(event);
                } catch (IllegalArgumentException e) {
                    System.out.println(e.getMessage());
                }
            } else if (header.equals("todo")) {
                ToDo todo = new ToDo(input.substring(header.length()));
